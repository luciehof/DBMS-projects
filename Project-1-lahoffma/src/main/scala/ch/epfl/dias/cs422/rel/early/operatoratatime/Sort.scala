...
class Sort protected (
  ...
  private def slicedSort(list: IndexedSeq[Tuple], fieldCollation: RelFieldCollation): IndexedSeq[Tuple] = {
  ...
  override def execute(): IndexedSeq[Column] = {
    ...
    if (inVector.dropRight(1).isEmpty) { IndexedSeq.empty }
      ...
      sorted = inVector.transpose
      fieldCollations.reverse.forEach(fieldCollation =>
        sorted = slicedSort(sorted.sortWith(orderFrom(fieldCollation)), fieldCollation)
      )
      sorted.transpose
    }
  }
  // TODO: optimize with offset and fetch
}
