...
class MainTest extends FunSuite {
  ...
  test("ExactNNDebug") {
    val data = spark.sparkContext
      .parallelize(List(
        "Star Wars IV|space|jedi|sci-fi|lightsaber",
        "Star Wars V|space|jedi|sci-fi|lightsaber",
        "Alien|sci-fi|space|horror",
        "Ghost in the Shell|sci-fi|dreams|thriller"
      ))
    val query = spark.sparkContext
      .parallelize(List(
    ...
    val bc = new ExactNN(spark.sqlContext, rddData, 0.6)
    val res = bc.eval(rddQuery)
      .collect()
    assert(res.contains(("Star Wars",Set("Alien"))))
    assert(res.length == 1)
    //println(bc.jaccard(Set("sci-fi","space","horror"),Set("sci-fi","space")).toString)
    //println((Set("sci-fi","space","horror") union Set("sci-fi","space")).size)
  }
  ...
  test("ANDConstruction") {
    ...
    println("base precision: ", Main.precision(ground, res1).toString)
    println("and precision: ", Main.precision(ground, res).toString)
